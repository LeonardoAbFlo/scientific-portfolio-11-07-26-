...
"""Create mitochondrial synteny plots by genus and for all samples.

The script compares genes at the nucleotide level. Reference mitogenomes can be
rotated to start 10 nucleotides after rrnS so circular genomes use a similar
origin. Rotated references are saved under ``_ref_rotated_rrnS``. References
without rrnS are used without rotation.

Input layouts:
  <root>/data/<genus>/<sample_dir>/*.gb
  <ref_dir>/<accession>-<taxon>/<accession>.gb
"""
...
class GeneHit:
    """Gene coordinates and sequence used in the comparison."""
    gene_id: str
    ftype: str
    seqid: str
    start_1based: int
...
def parse_args():
    p = argparse.ArgumentParser(
        description=(
            "Create mitochondrial synteny plots by genus and across all samples. "
            "References can be rotated to start 10 nucleotides after rrnS."
        )
    )

    p.add_argument(
        "--root",
        default="/path/to/mosquitos/results_p1",
        help="Project root (default: /path/to/mosquitos/results_p1)",
    )
    p.add_argument(
        "--data-subdir",
        default="CORRECTIONS_medaka_mitofinder_rrnS",
        help="Subdirectory containing genus folders",
    )
    p.add_argument(
        "--pattern",
    ...
    p.add_argument(
        "--ref-dir",
        default="/path/to/mosquitos/results_p1/data/ref_gb",
        help="Directory containing reference mitogenomes",
    )
    p.add_argument(
        "--ref-pattern",
        default="*.gb",
        help="GenBank pattern inside the reference directory (default: '*.gb')",
    )

    p.add_argument(
        "--no-rotate-ref-to-rrnS",
        action="store_true",
        help="Do not rotate references to start 10 nucleotides after rrnS",
    )
    p.add_argument(
        "--rrns-regex",
        default=r"(?:\brrnS\b|\b12S\b|12S[-\s]*rRNA|small[-\s]*subunit[-\s]*(?:ribosomal\s*)?RNA|s[-\s]*rRNA)",
        help="Regular expression used to detect rrnS or 12S rRNA qualifiers",
    )

    p.add_argument("--identity-thr", type=float, default=70.0, help="Minimum identity percentage")
    p.add_argument("--length-thr-nt", type=int, default=70, help="Minimum alignment length in nucleotides")

    p.add_argument("--barsize", type=int, default=2000, help="Scale-bar size in base pairs")
    ...
    p.add_argument("--show-trna-rrna", action="store_true", help="Draw tRNA and rRNA features and links")

    p.add_argument(
        "--outdir",
        default=None,
        help="Output directory (default: <root>/output/synteny)",
    )
    p.add_argument(
        "--prefix-all",
        default="all_synteny_nt",
        help="Output prefix for the combined plot",
    )
    p.add_argument(
        "--prefix-suffix",
        default="synteny_nt",
        help="Output suffix for each genus: <genus>_<suffix>",
    )
    return p.parse_args()
...
def list_genera(data_dir: Path) -> List[Path]:
    ...
    if not genera:
        sys.exit(f"No genus directories found in: {data_dir}")
    return genera
...
def add_identity_heatbar(fig, ax_ref, vmin, vmax, cmap, norm, title="Identity (%)"):
    """Add a vertical identity scale to the right of the main axis."""
    ...
    ax_bar.tick_params(axis="y", length=2.5, width=0.7, labelsize=9, pad=3)

    ax_bar.set_title(title, fontsize=9, pad=6)
...
def run_synteny_gene_links(
    ...
    if len(gb_paths) < 2:
        print(f"[SKIP] Fewer than two GenBank files: {out_png.name}", file=sys.stderr)
        return

    gb_paths = sorted(gb_paths, key=lambda p: track_label_plain_from_gb(p, data_dir, ref_dir=ref_dir).lower())
    ...
    if any_links:
        ...
        print(f"[WARN] No shared genes passed the filters. Plot saved: {out_png}")
...
def main():
    ...
    outdir = Path(a.outdir).expanduser().resolve() if a.outdir else (root / "output" / "synteny").resolve()
    ...
    if ref_dir.is_dir():
        ...
        if ref_gb_paths:
            print(f"[INFO] Reference genomes found: N={len(ref_gb_paths)} in {ref_dir}")
        else:
            print(f"[WARN] No GenBank files matching {a.ref_pattern} in {ref_dir}", file=sys.stderr)
    else:
        print(f"[WARN] Reference directory not found: {ref_dir}. Continuing without references.", file=sys.stderr)
        ref_dir = None
    ...
    if ref_dir and ref_gb_paths and (not a.no_rotate_ref_to_rrnS):
        ...
        print(f"[INFO] Rotating references to 10 nucleotides after rrnS: {rotated_ref_dir}")
        ...
        print("[INFO] Reference rotation disabled; using original GenBank files.")
    ...
    for genus_dir in genera:
        ...
        if not gb_paths:
            print(f"[WARN] {genus}: no GenBank files matching {a.pattern}", file=sys.stderr)
            continue
    ...
    all_gb = sorted(set(all_gb), key=lambda p: track_label_plain_from_gb(p, data_dir, ref_dir=ref_dir).lower())
    if len(all_gb) >= 2:
        ...
        print("[WARN] Fewer than two GenBank files; combined synteny plot not created.", file=sys.stderr)
